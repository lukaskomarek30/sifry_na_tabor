...
# family_index -> které strany rámečku se mají vykreslit
# top, right, bottom, left
FAMILY_LINES = {
    0: dict(top=False, right=True,  bottom=True,  left=False),  # ABC  ┘
    1: dict(top=False, right=True,  bottom=True,  left=True),   # DEF  ∪
    2: dict(top=False, right=False, bottom=True,  left=True),   # GHI  └
    3: dict(top=True,  right=True,  bottom=True,  left=False),  # JKL  ⊐
    4: dict(top=True,  right=True,  bottom=True,  left=True),   # MNO  □
    5: dict(top=True,  right=False, bottom=True,  left=True),   # PQR  ⊏
    6: dict(top=True,  right=True,  bottom=False, left=False),  # STU  ┐
    7: dict(top=True,  right=True,  bottom=False, left=True),   # VWX  ∩
    8: dict(top=True,  right=False, bottom=False, left=True),   # YZ   ┌
}
...
def tokenize_text(text: str) -> list[str]:
    """Rozdělí text na jednotlivé znaky. CH se tady nebere jako jeden znak."""
    return list(normalize_text(text))


def encrypt(text: str) -> str:
    """Připraví text pro kreslenou šifru Velký polský kříž (26 znaků)."""
...
if __name__ == "__main__":
    ...
    print("Dešifrování:", decrypt("[A][H][O][J] [J][A][K] [S][E] [M][A][S]?"))
